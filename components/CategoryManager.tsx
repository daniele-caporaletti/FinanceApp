...
export const CategoryManager: React.FC<CategoryManagerProps> = ({ categories, subcategories }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-sm border border-white/60 overflow-hidden flex flex-col md:flex-row min-h-[500px] md:min-h-[600px]">
      
      {/* --- Left Column: Categories List --- */}
      <div className="w-full md:w-1/3 border-r border-slate-200/60 flex flex-col max-h-[350px] md:max-h-full bg-white/40">
        <div className="p-3 md:p-4 border-b border-slate-200/60 flex justify-between items-center sticky top-0 bg-white/80 backdrop-blur z-10">
        ...
        {/* Add Category Form */}
        {isAddingCategory && (
          <div className="p-3 md:p-4 bg-indigo-50/50 border-b border-indigo-100 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
          {categories.length === 0 ? (
             ...
             categories.map(cat => (
               ...
               >
                 <span className={`font-semibold text-sm md:text-base ${selectedCategoryId === cat.id ? 'text-indigo-700' : 'text-slate-600'}`}>
                   {cat.name}
                 </span>
                 <ChevronRight size={14} className={`transition-transform duration-300 md:w-4 md:h-4 ${selectedCategoryId === cat.id ? 'text-indigo-400 translate-x-1' : 'text-slate-300'}`} />
               </button>
             ))
          )}
        </div>
      ...
      <div ref={rightColRef} className="w-full md:w-2/3 flex flex-col bg-white/60 min-h-[400px] border-t md:border-t-0 border-slate-200">
        {!selectedCategory ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-300 p-8">
            <div className="bg-slate-50 p-4 md:p-6 rounded-full mb-4 shadow-inner">
               <Folder size={32} className="md:w-[48px] md:h-[48px] text-slate-200" />
            </div>
            <p className="text-base md:text-lg font-bold text-slate-400">Seleziona una categoria</p>
          </div>
        ) : (
          <>
            <div className="p-3 md:p-4 border-b border-slate-200/60 bg-white/50 flex justify-between items-center h-[56px] md:h-[64px] sticky top-0 z-10 backdrop-blur-sm">
              <div className="flex items-center gap-2">
                 <span className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-wider flex items-center">
                    Cat <ChevronRight size={10} className="mx-1" />
                 </span>
                 <h3 className="font-bold text-slate-800 text-lg md:text-xl tracking-tight">{selectedCategory.name}</h3>
              </div>
              <button 
                onClick={() => setIsAddingSubcategory(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 bg-indigo-600 text-white text-[10px] md:text-xs font-bold rounded-full hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200 active:scale-95"
              >
            ...
            {isAddingSubcategory && (
              <div className="p-4 md:p-6 bg-slate-50 border-b border-slate-200 animate-in fade-in slide-in-from-top-2">
                 <div className="max-w-md bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <form onSubmit={handleAddSubcategory} className="flex gap-2 md:gap-3 items-end">
                      ...
                      <button 
                        ...
                        className="px-3 py-2 text-xs md:text-sm font-medium text-slate-500 hover:bg-slate-50 rounded-lg transition-colors"
                      >
                        Annulla
                      </button>
                      <button 
                        ...
                        className="px-4 py-2 text-xs md:text-sm font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 shadow-sm"
                      >
                        Salva
                      </button>
                    </form>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-6">
               {currentSubcategories.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-slate-400 text-sm rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 min-h-[200px]">
                    <Tag size={24} className="md:w-8 md:h-8 mb-2 text-slate-300" />
                    <p>Nessuna sottocategoria presente.</p>
                 </div>
               ) : (
                 <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3">
                    {currentSubcategories.map(sub => (
                       <div key={sub.id} className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:border-indigo-200 hover:shadow-md transition-all group">
                          <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                             <Tag size={12} className="md:w-[14px] md:h-[14px]" />
                          </div>
                          <span className="text-xs md:text-sm font-bold text-slate-700 truncate leading-tight">{sub.name}</span>
                       </div>
                    ))}
                 </div>
               )}
