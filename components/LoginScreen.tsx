...
import { ENCRYPTED_KEY } from '../constants';
    ...
    setTimeout(() => {
      if (password === 'FinanceApp') {
        try {
          ...
          setError('Errore critico durante la decrittazione.');
          setIsLoading(false);
        }
      } else {
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-indigo-600 p-8 flex flex-col items-center">
           ...
           <h1 className="text-2xl font-bold text-white tracking-tight">FinanceApp Security</h1>
           <p className="text-indigo-200 text-sm mt-1">Area Protetta</p>
        </div>

        <div className="p-8">
           <form onSubmit={handleLogin} className="flex flex-col gap-4">
             <div>
               <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Password di Accesso</label>
               <input 
                 type="password" 
                 value={password}
             ...
             >
               {isLoading ? 'Decrittazione in corso...' : (
           ...
           <p className="text-center text-xs text-slate-400 mt-6">
             Questa applicazione utilizza crittografia lato client.<br/>
             La chiave API è protetta fino all'inserimento della password.
           </p>
        </div>
      </div>
    </div>
