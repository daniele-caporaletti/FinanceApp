...
export const Categories: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      {/* VISTA MOBILE: CARDS ESPANDIBILI */}
      <div className="md:hidden space-y-4">
         {hierarchicalCategories.map(cat => {
            const isExpanded = expandedIds.has(cat.id);
            return (
               <div key={cat.id} className="bg-white border border-slate-100 rounded-[1.5rem] shadow-sm overflow-hidden">
                  {/* Header Categoria */}
                  <div 
                    onClick={() => cat.subcategories.length > 0 ? toggleExpand(cat.id) : null}
                    className="p-5 flex justify-between items-center bg-slate-50/50 active:bg-slate-100 transition-colors cursor-pointer"
                  >
                     <div className="flex items-center space-x-3">
                        <div className="w-2 h-2 rounded-full bg-blue-600"></div>
                        <span className="font-bold text-slate-900 text-lg">{cat.name}</span>
                        {cat.subcategories.length > 0 && <span className="text-xs font-bold text-slate-300">({cat.subcategories.length})</span>}
                     </div>
                     <div className="flex items-center space-x-1">
                        <button 
                           onClick={(e) => { e.stopPropagation(); setModalState({ open: true, parentId: cat.id }); }}
                        ...
                        <button 
                           onClick={(e) => { e.stopPropagation(); setModalState({ open: true, initialData: cat }); }}
                  ...
                  {isExpanded && (
                     <div className="bg-white border-t border-slate-100">
                        {cat.subcategories.map(sub => (
                           <div key={sub.id} className="flex justify-between items-center p-4 pl-10 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                              <div className="flex items-center space-x-2">
                              ...
                              <div className="flex items-center space-x-1">
                                 <button 
                                    onClick={() => setModalState({ open: true, initialData: sub })}
                                    className="p-2 text-slate-300 hover:text-blue-600 rounded-lg"
                                 >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                 </button>
                                 <button 
